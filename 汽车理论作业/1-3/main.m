...
title([num2str(properties.gearbox),'挡变速器下的驱动力-行驶阻力平衡图'])
xlabel('ua/(km/h)')
ylabel('Ft/N, (Ft+Fw)/N')
...
title([num2str(properties.gearbox),'挡变速器下的爬坡度图'])
xlabel('ua/(km/h)')
ylabel('i/(%)')
...
a = @(n,gearboxi)(Ft(n,gearboxi) - Fw(ua(n,gearboxi)) - Ff)/(kesi(gearboxi)*properties.mTotal);
...
title([num2str(properties.gearbox),'挡变速器下汽车加速度倒数曲线'])
xlabel('ua/(km/h)')
ylabel('1/a')
legend('1挡','2挡','3挡','4挡');
